...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        results["fixed_size"] = {
            "count": len(fc),
            "avg_length": sum(len(c) for c in fc) / len(fc) if fc else 0,
            "chunks": fc
        }
        ...
        results["by_sentences"] = {
            "count": len(sc),
            "avg_length": sum(len(c) for c in sc) / len(sc) if sc else 0,
            "chunks": sc
        }
        ...
        results["recursive"] = {
            "count": len(rc),
            "avg_length": sum(len(c) for c in rc) / len(rc) if rc else 0,
            "chunks": rc
        }
