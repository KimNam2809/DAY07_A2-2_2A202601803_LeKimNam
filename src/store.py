...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        embedding = self._normalize_vector(self._embedding_fn(doc.content))
        return {
            "id": doc.id,
            "content": doc.content,
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        ...
        query_embedding = self._normalize_vector(self._embedding_fn(query))
        ...
        if self._use_chroma and self._collection is not None:
            try:
                ...
                for record_id, metadata in zip(existing.get("ids", []), existing.get("metadatas", [])):
                    if record_id == doc_id or (metadata or {}).get("doc_id") == doc_id:
